...
fun StorageSettingsScreen(onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                SettingsInfoItem("Manage storage", "1.7 GB") {}
                HorizontalDivider(modifier = Modifier.padding(vertical = 16.dp), color = MaterialTheme.colorScheme.outline.copy(alpha = 0.2f))
            }

            item {
                SettingsInfoItem("Network usage", "4.0 GB sent • 7.1 GB received") {}
                SettingsSwitchItem("Use less data for calls", "", useLessData) { useLessData = it }
                SettingsInfoItem("Proxy", "Off") {}
            ...
            item {
                ...
                SettingsInfoItem("Media upload quality", "Standard quality") { /* TODO: Show Picker */ }
                SettingsInfoItem("Auto-download quality", "Choose...") { /* TODO: Show Picker */ }
                SettingsInfoItem("Media auto-download", "Voice messages are always automatically downloaded") {}
                SettingsInfoItem("When using mobile data", "Photos") {}
                SettingsInfoItem("When connected on Wi-Fi", "All media") {}
                SettingsInfoItem("When roaming", "No media") {}
            }
        }
    }
}
